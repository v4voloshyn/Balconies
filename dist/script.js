...
window.addEventListener('DOMContentLoaded', () => {
  ...
  Object(_modules_timer__WEBPACK_IMPORTED_MODULE_5__["default"])('#timer', '2021-06-25T23:59:59');
...
const changeModalState = state => {
  const windowForm = document.querySelectorAll('.balcon_icons_img'),
        ...
        windowProfile = document.querySelectorAll('.checkbox'); // Валидация инпутов для ввода только чисел

  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_0__["default"])('#width');
  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_0__["default"])('#height'); // выделение идентификатоора формы для дальнейшей передачи его в объект и на сервер???
...
const forms = state => {
  ...
  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_0__["default"])('input[name="user_phone"]'); // сообщения вывода пользователю в зависимости от результата отправки данных формы
  ...
  form.forEach(item => {
    item.addEventListener('submit', e => {
      e.preventDefault(); // вывод сообшений пользоваетлю

      let statusMessage = document.createElement('div');
      statusMessage.classList.add('status');
      item.appendChild(statusMessage); // функция закрития модального окна после успешной отправки
...
const images = () => {
  ...
  bigImage.classList.add('center-block-img');
...
const modals = () => {
  ...
  bindModals('.phone_link', '.popup', '.popup .popup_close'); //Открытие формы-калькулятора
...
const tabs = (headerSelector, tabSelector, contentSelector, activeClass, display = 'block') => {
  const header = document.querySelector(headerSelector),
        ...
        content = document.querySelectorAll(contentSelector); //скрыть табы
  ...
  hideTabContent();
  showTabContent(); // навешиваем обработчик клика на общую область, что соединяет табы

  header.addEventListener('click', e => {
    const target = e.target;

    if (target && (target.classList.contains(tabSelector.slice(1)) || //убираем точку с класса методом реплейс
    target.parentNode.classList.contains(tabSelector.slice(1)))) {
      //tabSelector.slice(1); //tabSelector.replace(/\./, ''))
      tab.forEach((item, i) => {
...
const timer = (id, deadline) => {
  //Добавлем ноль в начало числа таймера с одной цифрой
  const addZero = num => {
    if (num <= 9) {
      return '0' + num;
    } else {
  ...
  const setClock = (selector, endtime) => {
    const timer = document.querySelector(selector),
          ...
          timeInterval = setInterval(updateClock, 1000); //Вызываем функцию первый раз для избежания лага таймера
